...
class JHelperTags extends JHelper
{
	...
	public function addTagMapping($ucmId, JTableInterface $table, $tags = array())
	{
		...
		$query->insert('#__content_tags_map');
		$query->columns(
			array(
	...
	public function getItemTags($contentType, $id, $getTagData = true)
	{
		...
		$query = $db->getQuery(true)
			...
			->from($db->quoteName('#__content_tags_map') . ' AS m ')
			->where(
				array(
		...
		$query->join('INNER', $db->quoteName('#__content_tags') . ' AS t ' . ' ON ' . $db->quoteName('m.tag_id') . ' = ' . $db->quoteName('t.id'));
	...
	public function getTagIds($ids, $prefix)
	{
		...
		$query = $db->getQuery(true)
			...
			->from($db->quoteName('#__content_tags') . ' AS t ')
			->join(
				'INNER', $db->quoteName('#__content_tags_map') . ' AS m'
		...
		$anyOrAll = true, $languageFilter = 'all', $stateFilter = '0,1')
	{
		...
		$query
			...
			->join(
				'INNER',
				'#__content_ucm_core AS c ON m.type_alias = c.core_type_alias AND m.core_content_id = c.core_content_id AND c.core_state IN ('
	...
	public function getTagNames($tagIds)
	{
		...
		if (is_array($tagIds) && count($tagIds) > 0)
		{
			...
			$query = $db->getQuery(true)
				...
				->from($db->quoteName('#__content_tags'))
	...
	public static function searchTags($filters = array())
	{
		...
		$query = $db->getQuery(true)
			...
			->join('LEFT', $db->quoteName('#__content_tags', 'b') . ' ON a.lft > b.lft AND a.rgt < b.rgt');
	...
	public function tagDeleteInstances($tag_id)
	{
		...
		$db = JFactory::getDbo();
		$query = $db->getQuery(true)
			->delete($db->quoteName('#__content_tags_map'))
